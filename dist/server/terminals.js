...
async function readCommandsFile(path) {
    ...
    try {
        ...
        return {
            commands: [],
            warning: `Failed to read commands file: ${err.message}`,
        };
    }
    let parsed;
    try {
...
export async function saveCommandsFile(workspaceRoot, commands) {
    ...
    try {
        ...
        return { path, error: `Failed to save commands file: ${err.message}` };
    }
}
...
export function encodeTerminalKey(key, modifiers = {}) {
    ...
    if (!data)
        return { error: `Unsupported terminal key: ${key}` };
    ...
    if (arrow && modifier !== 1) {
        ...
        if (modifiers.ctrl) {
            if (key.length !== 1)
                return { error: `Invalid Ctrl combo: ${key}` };
...
export class TerminalManager {
    ...
    create(id, cwd, cols, rows, fallbackCwd, title, opts) {
        ...
        if (this.spawnShell(id, safeCwd, cols, rows, title || `Terminal ${++this.seq}`, undefined, opts?.forceBash)) {
    ...
    spawnShell(id, cwd, cols, rows, title, command, forceBash) {
        ...
        try {
            ...
            this.fail(id, helper
                ? `Failed to start terminal: ${err.message} (node-pty spawn-helper is not executable; run: chmod +x "${helper}")`
    ...
    appendOutput(entry, data) {
        ...
        if (entry.watches.length > 0) {
            ...
            for (const w of entry.watches) {
                ...
                if (m)
                    hits.push({ w, m }); // hit → remove (cb fired together below)
                else
                    remaining.push(w);
            }
    ...
    inputChecked(id, data) {
        if (data.length > MAX_INPUT)
            return `Input too long (max ${MAX_INPUT} chars)`;
        const entry = this.terms.get(id);
        if (!entry || entry.exited)
            return "Terminal does not exist or the process has exited";
            ...
            try {
                ...
                for (;;) {
                    ...
                    if (deadline !== null && Date.now() > deadline) {
                        terminals.setSentinelPending(TERM_ID, false);
                        terminals.inputChecked(TERM_ID, "\x03");
                        throw new Error(`Command timed out after ${p.timeout}s (sent Ctrl+C; output so far: ${truncateMiddle(stripAnsi(collected), 4000)})`);
                    }
...
function backgroundResult(terminals, opts, command, partialText, silentSeconds) {
    ...
    return {
        content: [
            {
                type: "text",
                text: `Command is still running in persistent terminal ai-bash (no output for ${silentSeconds}s, not finished).` +
    ...
    return [
        defineTool({
            ...
            execute: async (_id, p) => {
                ...
                if (!info)
                    throw new Error(`Failed to create terminal: ${p.terminalId}`);
