...
export async function isWorkingTreeDirty(root) {
    try {
...
export async function hasHead(root) {
    try {
...
export async function getRepoInfo(root, baseBranch) {
...
export async function getDiff(root, mode, baseBranch) {
    ...
    const baseSha = baseRef === "HEAD" && repo.headSha === "0".repeat(40)
        ? repo.headSha
    ...
    if (mode === "working-tree") {
        ...
        diffText = await runGit(["diff", "--no-ext-diff", "--no-color", "-M", `${mergeBase}...HEAD`], root);
    }
...
function isSkippedUntracked(filePath) {
    return filePath.split(/[/\\]/).some((segment) => segment === "node_modules" || segment === ".local-review");
}
function resolveBaseBranch(current, branches, requested) {
