...
export class ClientSession {
    ...
    settingsSvc; // created in the constructor (needs clientId/stateStore)
    ...
    notifyTerminalIdle(conversationId, terminalId, idleMs, title) {
        ...
        void conv.runtime.session
            .sendUserMessage(`(System: the terminal "${title}" you started has had no new output for ${seconds} seconds.` +
    ...
    notifyTerminalBashDone(terminals, info) {
        ...
        const text = `(System: the command you left running in terminal ${info.terminalId} finished (${exitText}): ${cmdShort}\n` +
    ...
    async installPiAgent() {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `pi agent install failed: ${err.message}`,
            });
        }
        this.flushSnapshot();
    }
        ...
        execPluginCommand: async (name, args) => {
            ...
            try {
                ...
                this.emit({
                    type: "notice",
                    level: "error",
                    text: `Plugin command /${name} failed: ${err.message}`,
                });
            }
            return true;
        },
    ...
    queue = false) {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to send prompt: ${err.message}`,
            });
        }
    ...
    async abortBash() {
        ...
        try {
            await this.conv.runtime.session.sendUserMessage("(System: the user stopped the bash command. Output up to abort is in the tool result. Continue from that; do not rerun it unless necessary.)");
        }
        catch {
    ...
    async interruptRun(conv, reason) {
        ...
        const force = () => {
            if (forced)
                return;
            forced = true;
            void this.forceResetConversation(conv, `${reason}: run did not stop, forced a reset of this conversation`);
        };
    ...
    async forceResetConversation(conv, reason) {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Force-interrupt failed: ${err.message}`,
            });
        }
    }
    async newChat() {
    ...
    async editMessage(messageId, text, attachments) {
        ...
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Edit & re-ask failed: ${err.message}`,
            });
        }
        this.flushSnapshot();
    }
    ...
    async reviewDiff(reqId, mode, base) {
    ...
    async reviewSubmit(reqId, mode, baseBranch, comments) {
    ...
    async reviewApply() {
        ...
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to apply Local Review: ${err.message}`,
            });
        }
    }
    async maybeNudgeReview() {
        try {
            const { pendingReviewSummary } = await import("./review/review.js");
            const summary = await pendingReviewSummary(this.cwd);
    ...
    setSessionName(name) {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to name session: ${err.message}`,
            });
        }
    }
    emitSessionTree() {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to read session tree: ${err.message}`,
            });
        }
    }
    async navigateTree(entryId) {
        ...
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to jump in the session tree: ${err.message}`,
            });
        }
    }
    async exportSession() {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to export session: ${err.message}`,
            });
        }
    }
    bundledPromptPaths() {
    ...
    async setCwd(newCwd) {
        try {
            ...
            this.emit({
                type: "notice",
                level: "error",
                text: `Failed to switch workspace: ${err.message}`,
            });
        }
        this.flushSnapshot();
    }
