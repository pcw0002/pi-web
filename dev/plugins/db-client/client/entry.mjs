/* db-client client bundle — generated by npm run build; source is src/client.js */
...
<div class="dbx">
    ...
    function renderConns() {
      ...
      for (const c of state.conns) {
        ...
        row.innerHTML = `<span class="dot ${isActive ? "on" : ""}"></span><span class="info"><span class="nm">${esc(c.name)}</span><span class="addr">${esc(connAddr(c))}</span></span><span class="ops"><button data-op="edit" title="Edit">✎</button><button data-op="del" title="Delete">🗑</button></span>`;
    ...
    async function refreshTables() {
      ...
      if (!r.ok) {
        toast(`Failed to list tables: ${r.error}`, true);
        work.tables = [];
    ...
    async function loadData() {
      ...
      statusLine.textContent = `${work.curTable} · ${fmtCount(p.total)} rows` + (r.grid.rows.length ? ` · ${r.grid.rows.length} on this page` : "");
    ...
    function renderGrid(host, columns, rows, kind, grid = {}) {
      ...
      rows.forEach((row) => {
        ...
        }).join("") + (canDel ? '<td class="ops-cell">' + (kind === "mongodb" ? '<button data-op="docedit" title="Edit document">✎</button>' : "") + '<button data-op="del" title="Delete">🗑</button></td>' : "");
        tb.appendChild(tr);
      });
      tbl.appendChild(tb);
      if (canEditCell) {
    ...
    async function confirmDeleteRow(tr) {
      ...
      if (work.kind === "mongodb") {
        ...
        if (!pkCell) {
          toast("This table has no primary key, so the row cannot be located", true);
          return;
        }
    ...
    async function openInsertModal() {
      ...
      if (!work.schema) {
        ...
        if (!r.ok) {
          toast(`Failed to read schema: ${r.error}`, true);
          return;
        }
      ...
      rowBody.innerHTML = `<div class="flds">${cols.map((c) => `<label>${esc(c.name)}<small>${esc(c.type)}${c.nullable ? "" : " · not null"}</small></label><input data-col="${esc(c.name)}" spellcheck="false" placeholder="${c.nullable ? "blank = default" : "required"}" />`).join("")}</div>`;
    ...
    async function loadSchema() {
      ...
      if (d.columns.length) {
        ...
        t.className = "mtable";
        t.innerHTML = "<thead><tr><th>#</th><th>Column</th><th>Type</th><th>Null</th><th>Key</th><th>Default</th><th>Comment</th></tr></thead><tbody>" + d.columns.map((c, i) => `<tr><td>${i + 1}</td><td>${esc(c.name)}</td><td>${esc(c.type)}</td><td>${c.nullable ? "YES" : "NO"}</td><td>${c.key ? `<span class="keytag">${esc(c.key)}</span>` : ""}</td><td>${esc(c.def ?? "")}</td><td>${esc(c.comment || "")}</td></tr>`).join("") + "</tbody>";
      ...
      if (d.indexes?.length) {
        const t = document.createElement("table");
        t.className = "mtable";
        t.innerHTML = "<thead><tr><th>Index</th><th>Unique</th><th>Columns / definition</th></tr></thead><tbody>" + d.indexes.map((i) => `<tr><td>${esc(i.name)}</td><td>${i.unique ? "✓" : ""}</td><td>${esc(i.columns)}</td></tr>`).join("") + "</tbody>";
    ...
    async function runQuery() {
      ...
      qStatus.textContent = `✓ ${r.grid.elapsedMs}ms` + (r.grid.total ? ` · ${r.grid.total} rows` : "") + (r.grid.affected ? ` · ${r.grid.affected} affected` : "");
    ...
    async function redisScan(pattern) {
      ...
      if (!r.keys.length) {
        keysList.innerHTML = "<div class='grid-empty'>No matching keys</div>";
        return;
      }
